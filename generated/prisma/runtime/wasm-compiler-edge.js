	...
	c = ye(() => {
		h = class e extends Uint8Array {
			...
			toString(t = "utf8", r = 0, n = this.length) {
				...
				if (t === "base64" || t === "base64url") {
					const i = btoa(this.reduce((o, s) => o + ui(s), ""));
					return t === "base64url"
...
var mt,
	...
	Yo = ye(() => {
		...
		I.logarithm = I.log = function (e) {
			...
			return this.eq(he)
				? new n(0)
				: ((W = !1), (t = _e(Wt(this, o), Wt(e, o), o)), (W = !0), q(t, i));
		};
	...
	const t = {
		...
		r = (...n) => {
			...
			if (
				...
				const f = n.map((A) => (typeof A == "string" ? A : vu(A))),
					E = `+${Date.now() - ps}ms`;
				(ps = Date.now()), a(o, ...f, E);
			}
		};
...
function Ra({ result: e, modelName: t, select: r, omit: n, extensions: i }) {
	...
	for (const f of Object.values(o)) {
		if (n) {
			...
			const E = f.needs.filter((A) => n[A]);
			E.length > 0 && a.push(It(E));
		} else if (r) {
			if (!r[f.name]) continue;
			const E = f.needs.filter((A) => !r[A]);
			E.length > 0 && a.push(It(E));
		}
...
var rl = 2,
	...
	Om = tt(
		nl
			.split("")
			.reduce((e, t) => +e + t.charCodeAt(0), +km + 36)
			.toString(36),
		rl,
	);
function Ki() {
...
function Fd(e, t, r) {
	let n = 0,
		i = 0;
	for (const s of Eo(e, t, void 0)) {
		let a = 0;
		for (const f of ic(s)) a++;
		(i = Math.max(i, a)), (n += a);
	}
	let o = [[]];
	for (const s of Eo(e, t, void 0))
		...
		switch (t.type) {
			...
			case "mapRecord": {
				const { value: o, lastInsertId: s } = await this.interpretNode(
					...
					a = o === null ? {} : vo(o);
				for (const [f, E] of Object.entries(t.args.fields))
					a[f] = Hd(E, a[f], n, i);
...
function ri(e) {
	...
	g.env.TEST_CLIENT_ENGINE_REMOTE_EXECUTOR &&
		...
		(f = "http:");
	const E = new URL(i.href.replace(o, f));
			...
			return (
				Ro === void 0 &&
					(Ro = (async () => {
						...
						const o = { "./query_compiler_bg.js": n },
...
async function hf(e, t) {
	const r = xc["@prisma/engines-version"],
...
`);
	...
	const C = { accelerate: S, ppg: E, driverAdapters: A };
	function L(k) {
		return k.length > 0;
	}
	return L(o)
...
function Rc({ copyEngine: e = !0 }, t) {
	...
	for (const R of o.warnings) tr(...R.value);
	if (!n) {
		const R = o.errors[0];
...
function _c(e) {
	...
	const t = {};
	for (const r of Object.keys(e)) t[r] = Oc(e[r]);
	return t;
}
function Oc(e) {
...
var xf = bi.split(".")[0],
	...
	Uo = class {
		...
		getGlobalTracingHelper() {
			const t = globalThis[`V${xf}_PRISMA_INSTRUMENTATION`],
	...
	ai = class {
			...
			(this.logEmitter = r),
				...
				(this.dataloader = new oi({
					...
					singleLoader: async (n) => {
						const i =
								n.transaction?.kind === "itx" ? Bc(n.transaction) : void 0,
		...
		handleRequestError({
			...
			if (
				...
				const E = s ? { modelName: s, ...t.meta } : t.meta;
				throw new X(f, {
					code: t.code,
					clientVersion: this.client._clientVersion,
					meta: E,
	...
	If = {
		datasources: (e, { datasourceNames: t }) => {
			if (e) {
				...
				for (const [r, n] of Object.entries(e)) {
					if (!t.includes(r)) {
						const i = Jt(r, t) || ` Available datasources: ${t.join(", ")}`;
						throw new V(
...
Expected string or undefined.`);
		...
		omit: (e, t) => {
			...
			for (const [n, i] of Object.entries(e)) {
				const o = Of(n, t.runtimeDataModel);
				if (!o) {
					r.push({ kind: "UnknownModel", modelKey: n });
					continue;
				}
				for (const [s, a] of Object.entries(i)) {
					const f = o.fields.find((E) => E.name === s);
					if (!f) {
...
function kf(e, t) {
	...
	const r = t.map((i) => ({ value: i, distance: (0, Kc.default)(e, i) }));
	...
	class t {
		...
		async _transactionWithCallback({ callback: n, options: i }) {
			...
			try {
				...
				throw (
					(await this._engine.transaction("rollback", o, a).catch(() => {}), E)
				);
			}
			return f;
		}
		...
		_request(n) {
			...
			const i = n.middlewareArgsMapper ?? _f,
				o = {
				...
				a = async (f) => {
					const { runInTransaction: E, args: A, ...R } = f,
						S = { ...n, ...R };
